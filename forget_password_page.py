...
class forget_Page:
    ...
    def __init__(self, window):
        ...
        self.continue_btn = Button(
            ...
            bd=0,
            command=self.email_checker,
        )
        self.continue_btn.place(x=235, y=322, width=100, height=30)

        # ------------------ note -----------------
        self.note_label = Label(
            self.lgn_frame, text="📝", font=("Segoe UI", 25), bg="#312c3c", fg="#F6069A"
        )
        self.note_label.place(x=70, y=245)
        self.note_text = Label(
            self.lgn_frame,
            text="We’ll send your credentials to your email.",
            font=("Snap ITC", 12, "normal"),
            bg="#312c3c",
            fg="#FFFFFF",
            width=0,
            height=0,
    ...
    def email_checker(self):
        ...
        if self.is_valid_email() == True:
            if self.found_email() == False:
                ...
                the_username = c.execute(
                    "SELECT username FROM users WHERE email =?", (the_email,)
                ).fetchone()[0]
                the_password = c.execute(
                    "SELECT password FROM users WHERE email =?", (the_email,)
                ).fetchone()[0]
