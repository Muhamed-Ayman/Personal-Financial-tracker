...
class LoginPage:
    ...
    def Confirmpasswordd(self):
        if(self.Confirmpassword_entry.get() != self.password_entry.get()):
            self.Confirmpassword_bg.place(x=60, y=507)
            self.confirm_wrong_label.place(x=65, y=544)
            print("not match") 
        else:
    ...
    def credential_output(self):
      
      list_of_credentials={"username": self.username_entry.get(), "password": self.password_entry.get()}
