...
def log_transaction(trans_id, action):
    ...
    c.execute(
        "INSERT INTO logs (trans_id, action, timestamp) VALUES (?, ?, ?)",
...
def resource_path(relative_path):
    try:
        ...
        base_path = os.path.dirname(os.path.abspath(__file__))  # Correct project folder
...
class main_app:
    def __init__(self, window, username=None):
        ...
        # transaction bg
        imgtrans = Image.open(resource_path("images/transaction_bg.png"))
        imgtrans = imgtrans.resize((1535, 842), Image.Resampling.LANCZOS)
        self.trans_bg = ImageTk.PhotoImage(imgtrans)
        self.transaction_frame = Frame(
            self.window, bg="#4e2099", width=1535, height=842
        )
        self.transaction_frame.place(x=0, y=0)
        self.main_image_label = Label(
            self.transaction_frame, image=self.trans_bg, bg="#2d2638"
        )
        ...
        self.main_image_dashboard_label = Label(
            self.dashboard_frame, image=self.dash_bg, bg="#2d2638"
        )
        ...
        self.main_image_history_label = Label(
            self.history_frame, image=self.hist_bg, bg="#2d2638"
        )
        self.main_image_history_label.place(x=0, y=0)

        # main buttons for trnasaction page
        self.dashboard_btn = Label(
            self.transaction_frame,
            text="D a s h b o a r d",
            fg="#292828",
            bg="#6a5b72",
            font=("Britannic Bold", 25, "bold"),
            cursor="hand2",
        )
        ...
        self.profit_radio = Radiobutton(
            self.transaction_frame,
            text="➕ Profit (Addition)",
            variable=self.transaction_type,
            value="Profit",
            bg="#2d2638",
            fg="#4CAF50",
            font=("poppins", 12, "bold"),
            activebackground="#2d2638",
            activeforeground="#4CAF50",
            selectcolor="#4a4054",
        )
        self.profit_radio.place(x=245, y=395)

        self.loss_radio = Radiobutton(
            self.transaction_frame,
            text="➖ Loss (Subtraction)",
            variable=self.transaction_type,
            value="Loss",
            bg="#2d2638",
            fg="#FF5252",
        ...
        self.date_icon = Label(
            self.transaction_frame,
            bg="#2d2638",
            text="📅",
            fg="#b8b8b8",
            font=("poppins", 17, "normal"),
        )
        ...
        month = now.month
        day = now.day
        self.the_calender = Calendar(
            self.history_frame, selectmode="day", year=year, month=month, day=day
        )
        self.the_calender.place(x=92, y=260)
        self.collect_date_btn = Button(
            self.history_frame,
            text="search by day",
            cursor="hand2",
            bg="#2d2638",
            font=("poppines", 15, "bold"),
        ...
        previous_arrow_img = previous_arrow_img.resize(
            (46, 46), Image.Resampling.LANCZOS
        )
        previous_arrow_img = ImageTk.PhotoImage(previous_arrow_img)
        self.previous_arrow_btn = Button(
            self.history_frame,
        ...
        self.the_total = Label(
            self.history_frame,
            text=f"{len(self.total_logs)}",
            bg="#ffffff",
            fg="#000000",
            font=("Britannic Bold", 20, "bold"),
        )
        self.the_total.place(x=860, y=235)
        self.the_total2 = Label(
            self.history_frame,
            text=f"",
            bg="#ffffff",
            fg="#000000",
            font=("Franklin Gothic Mediu...", 18, "normal"),
        )
    ...
    def submit_transaction(self):
        ...
        if category == "Choose a Category":
            messagebox.showerror(
                "Error", "Please select a category!", parent=self.window
            )
            return
        if not amount_text:
        ...
        try:
            ...
            if amount <= 0:
                messagebox.showerror(
                    "Error", "Amount must be greater than 0!", parent=self.window
                )
                return
        except ValueError:
        ...
        try:
            ...
            # Log the transaction
            log_transaction(
                trans_id, f"Added {trans_type} of {amount} (category '{category}')"
            )
            ...
            messagebox.showerror(
                "Database Error", f"Failed to save transaction:\n{str(e)}"
            )
    ...
    def looping_table_show(self):
        ...
        self.previous_arrow_btn.config(state="normal" if self.start > 0 else "disabled")
    ...
    def draw_daily_expenses_graph(self, daily_expenses_data):
        ...
        # ---------------- Line + Fill ----------------
        ax.plot(dates, totals, marker="o", linewidth=2.5, color="#C77DFF", markersize=6)

        ax.fill_between(dates, totals, color="#9D4EDD", alpha=0.25)

        # ---------------- Titles & Labels ----------------
        ax.set_title(
            "Daily Expenses", fontsize=14, fontweight="bold", color="#E0AAFF", pad=15
        )
        ...
        # ---------------- Grid & Ticks ----------------
        ax.grid(True, linestyle="--", linewidth=0.6, alpha=0.4, color="white")
        ...
        canvas.get_tk_widget().place(x=150, y=382, width=800, height=415)
